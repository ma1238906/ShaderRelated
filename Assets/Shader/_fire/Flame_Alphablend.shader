...
SubShader {
	...
	VerttoFrag vert(ApptoVert v)
	{
		...
		_WindPower.xyz =normalize( mul((float3x3)unity_WorldToObject,_WindPower.xyz));
		...
		fixed3 normalDir = normalize(mul(unity_ObjectToWorld ,fixed4(v.inNormal.xyz,0) ).xyz);
